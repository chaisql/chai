// Generated file from "modernc.org/b" package

// Copyright 2014 The b Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package internal // import "modernc.org/b"
...
func (t *Tree) extract(q *d, i int) { // (r record.Record) {
	t.ver++
...
func (t *Tree) First() (k []byte, v record.Record) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k []byte) (v record.Record, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k []byte, v record.Record) *d {
	t.ver++
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k []byte, v record.Record) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k []byte, v record.Record) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *Tree) Set(k []byte, v record.Record) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *Tree) Put(k []byte, upd func(oldV record.Record, exists bool) (newV record.Record, write bool)) (oldV record.Record, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV record.Record
	if q == nil {
...
func (t *Tree) split(p *x, q *d, pi, i int, k []byte, v record.Record) {
	t.ver++
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k []byte, v record.Record, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k []byte, v record.Record, err error) {
	if err = e.err; err != nil {
		return
	}
