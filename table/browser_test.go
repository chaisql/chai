...
func TestBrowser(t *testing.T) {
	...
	t.Run("ForEach", func(t *testing.T) {
		...
		t.Run("Error", func(t *testing.T) {
			...
			b2 := b.ForEach(func(r record.Record) error {
				return err
			})
	...
	t.Run("Filter", func(t *testing.T) {
		// filter odd ids
		b2 := b.Filter(func(r record.Record) (bool, error) {
			f, err := r.Field("id")
			require.NoError(t, err)
			v, err := field.DecodeInt64(f.Data)
			require.NoError(t, err)
		...
		t.Run("OK", func(t *testing.T) {
			...
			count := 0
			b := b2.ForEach(func(r record.Record) error {
				f, err := r.Field("id")
				require.NoError(t, err)
				v, err := field.DecodeInt64(f.Data)
				require.NoError(t, err)
	...
	t.Run("Map", func(t *testing.T) {
		// double the age
		b2 := b.Map(func(r record.Record) (record.Record, error) {
			f, err := r.Field("age")
			require.NoError(t, err)
	...
	t.Run("GroupBy", func(t *testing.T) {
		t.Run("Ok", func(t *testing.T) {
			...
			for i, b := range g.Readers {
				b.ForEach(func(r record.Record) error {
					f, err := r.Field("group")
					require.NoError(t, err)
