...
func (p *Parser) parseUnaryExpr(allowed ...scanner.Token) (expr.Expr, error) {
	...
	case scanner.NAMEDPARAM:
		if len(lit) == 1 {
			return nil, errors.New(&ParseError{Message: "missing param name"})
		}
		if p.orderedParams > 0 {
			return nil, errors.New(&ParseError{Message: "cannot mix positional arguments with named arguments"})
		}
		p.namedParams++
	...
	case scanner.NUMBER:
		...
		if err != nil {
			return nil, errors.New(&ParseError{Message: "unable to parse number", Pos: pos})
		}
	...
	case scanner.ADD, scanner.SUB:
		...
		if tok != scanner.NUMBER && tok != scanner.INTEGER {
			return nil, errors.New(&ParseError{Message: "syntax error", Pos: pos})
		}
		if sign == scanner.SUB {
			lit = "-" + lit
		}
	...
	case scanner.INTEGER:
		...
		if err != nil {
			...
			return nil, errors.New(&ParseError{Message: "unable to parse integer", Pos: pos})
		}
