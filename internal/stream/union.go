...
func (it *UnionIterator) Next() bool {
	...
	for _, s := range it.streams {
		if err := it.iterateOnStream(s, it.temp); err != nil {
			it.err = err
			return false
		}
	}
...
func (it *UnionIterator) iterateOnStream(s *Stream, temp *tree.Tree) error {
...
func (it *UnionIterator) Key() (*tree.Key, error) {
	if it.err != nil {
		return nil, it.err
	}
	...
	return it.tempIt.Key(), nil
}

func (it *UnionIterator) Row() (row.Row, error) {
...
func (it *UnionIterator) TableName() (string, error) {
	panic("non implemented in union operator")
}

func (it *UnionIterator) Env() *environment.Environment {
	return it.env
}
