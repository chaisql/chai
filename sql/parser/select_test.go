...
func TestParserSelect(t *testing.T) {
	tests := []struct {
		...
		{"NoTable", "SELECT 1",
			query.SelectStmt{
				Selectors: []query.ResultField{query.ResultFieldExpr{Expr: query.IntValue(1), ExprName: "1"}},
			}, false},
		...
		{"WithExpr", "SELECT a    > 1 FROM test",
			query.SelectStmt{
				Selectors: []query.ResultField{query.ResultFieldExpr{Expr: query.Gt(query.FieldSelector([]string{"a"}), query.IntValue(1)), ExprName: "a    > 1"}},
				TableName: "test",
			}, false},
